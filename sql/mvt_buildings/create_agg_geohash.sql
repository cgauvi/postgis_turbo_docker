...
FUNCTION public.create_agg_geohash_tbl( geohash_prec integer,
									   tbl_name varchar default 'postgisftw.building_footprints_open_data' , 	
...
BEGIN
	...
	sql_cmd := 'create INDEX  if not exists postgisftw_building_footprints_open_data_proj_geo_' || geohash_prec || '_idx 
...
(
	select st_multi(
		...
		) as geom, gid, table_orig
	from postgisftw.building_footprints_open_data
);
-- #
